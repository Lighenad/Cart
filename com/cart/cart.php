...
function ViewCart($dni,$password,$discount = '0'){
    if(Login($dni,$password)){
        $xml = _GetCart();//En caso de que no exista un archivo cart.xml creo un archivo cart.xml vacio para que no de error
        $xml->asXML('xmldb/cart.xml'); //Guardos los cambios en el nuevo archivo cart.xml
        $counter = 1;
        $product_number = 1;
        $subtotal_cost = 0;
        $total_cost = 0;
